...
namespace Server_Strategico.Gioco
{
    public class AttacchiCooperativi
    {
        ...
        public class AttaccoCooperativo
        {
            ...
            public bool AggiungiGiocatore(string username, TruppeContribuite truppe)
            {
                ...
                if (GiocatoriPartecipanti.ContainsKey(username))
                {
                    ...
                    truppeEsistenti.Guerrieri += truppe.Guerrieri;
                    truppeEsistenti.Lancieri += truppe.Lancieri;
                    truppeEsistenti.Arcieri += truppe.Arcieri;
                    truppeEsistenti.Catapulte += truppe.Catapulte;
                }
                else
        ...
        public class TruppeContribuite
        {
            ...
            public TruppeContribuite(int guerrieri, int lancieri, int arcieri, int catapulte, string username)
            {
                Player = username;
                Guerrieri = guerrieri;
                Lancieri = lancieri;
        ...
        public static bool PartecipaDiAttacco(string idAttacco, string username, int guerrieri, int lancieri, int arcieri, int catapulte, Guid clientGuid)
        {
            ...
            player.Lancieri -= lancieri;
            player.Arceri -= arcieri;
            player.Catapulte -= catapulte;
            ...
            {
                totGuerrieri += part.Value.Guerrieri;
                totLancieri += part.Value.Lancieri;
                totArcieri += part.Value.Arcieri;
                totCatapulte += part.Value.Catapulte;
            }
        ...
        public static bool AbbandoaDiAttacco(string idAttacco, string username, Guid clientGuid)
        {
            ...
            if (player != null)
            {
                player.Guerrieri += truppeContribuite.Guerrieri;
                player.Lancieri += truppeContribuite.Lancieri;
                player.Arceri += truppeContribuite.Arcieri;
                player.Catapulte += truppeContribuite.Catapulte;
            }
        ...
        public static void GetListaAttacchi(Guid clientGuid)
        {
            ...
            foreach (var attaccoInfo in AttacchiInCorso)
            {
                ...
                {
                    gTot += part.Value.Guerrieri;
                    lTot += part.Value.Lancieri;
                    aTot += part.Value.Arcieri;
        ...
        private static async Task EseguiBattagliaCooperativa(string idAttacco, Guid clientGuid)
        {
            ...
            int totaleGuerrieri = 0;
            int totaleLancieri = 0;
            int totaleArcieri = 0;
            int totaleCatapulte = 0;
            int totaleFrecce = 0;
            ...
            int picchieri = playerVirtuale.Lancieri;
            ...
            int picchieri_Temp = Battaglie.RidurreNumeroSoldati(picchieri, dannoInflittoDalNemico, Esercito.Unità.Lanciere.Difesa * picchieri, Esercito.Unità.Lanciere.Salute);
            int arcieri_Temp = Battaglie.RidurreNumeroSoldati(arcieri, dannoInflittoDalNemico * 0.70, Esercito.Unità.Arciere.Difesa * arcieri, Esercito.Unità.Arciere.Salute);
            int catapulte_Temp = Battaglie.RidurreNumeroSoldati(catapulte, dannoInflittoDalNemico, Esercito.Unità.Catapulta.Difesa * catapulte, Esercito.Unità.Catapulta.Salute);

            // Applicare il danno alle unità nemiche
            int guerrieri_Enemy_Temp = Battaglie.RidurreNumeroSoldati(guerrieri_Enemy, dannoInflitto, Esercito.EsercitoNemico.Guerriero.Difesa * guerrieri_Enemy, Esercito.EsercitoNemico.Guerriero.Salute);
            int picchieri_Enemy_Temp = Battaglie.RidurreNumeroSoldati(picchieri_Enemy, dannoInflitto, Esercito.EsercitoNemico.Lanciere.Difesa * picchieri_Enemy, Esercito.EsercitoNemico.Lanciere.Salute);
            int arcieri_Enemy_Temp = Battaglie.RidurreNumeroSoldati(arcieri_Enemy, dannoInflitto, Esercito.EsercitoNemico.Arciere.Difesa * arcieri_Enemy, Esercito.EsercitoNemico.Arciere.Salute);
            int catapulte_Enemy_Temp = Battaglie.RidurreNumeroSoldati(catapulte_Enemy, dannoInflitto, Esercito.EsercitoNemico.Catapulta.Difesa * catapulte_Enemy, Esercito.EsercitoNemico.Catapulta.Salute);
            ...
            foreach (var partecipante in attacco.GiocatoriPartecipanti)
            {
                ...
                if (giocatore != null && giocatore.guid_Player != Guid.Empty)
                {
                    ...
                    double rapportoLancieri = totaleLancieri > 0 ? (double)partecipante.Value.Lancieri / totaleLancieri : 0;
                    double rapportoArcieri = totaleArcieri > 0 ? (double)partecipante.Value.Arcieri / totaleArcieri : 0;
                    double rapportoCatapulte = totaleCatapulte > 0 ? (double)partecipante.Value.Catapulte / totaleCatapulte : 0;
                    ...
                    int guerrieriRestituiti = partecipante.Value.Guerrieri - guerrieriPersiGiocatore;
                    int lancieriRestituiti = partecipante.Value.Lancieri - lancieriPersiGiocatore;
                    int arcieriRestituiti = partecipante.Value.Arcieri - arcieriPersiGiocatore;
                    int catapulteRestituite = partecipante.Value.Catapulte - catapultePersiGiocatore;
                    double frecceRestituite = giocatore.Frecce - freccePersiGiocatore;

                    giocatore.Guerrieri += guerrieriRestituiti;
                    giocatore.Lancieri += lancieriRestituiti;
                    giocatore.Arceri += arcieriRestituiti;
                    ...
                    if (attaccoCooperativo.TempoRimanente <= 0)
                    {
                        ...
                        foreach (var partecipante in attaccoCooperativo.GiocatoriPartecipanti)
                        {
                            ...
                            if (giocatore != null)
                            {
                                ...
                                giocatore.Lancieri += partecipante.Value.Lancieri;
                                giocatore.Arceri += partecipante.Value.Arcieri;
                                giocatore.Catapulte += partecipante.Value.Catapulte;
        ...
        public static void GetMieiAttacchi(string username, Guid clientGuid)
        {
            ...
            foreach (var attacco in AttacchiInCorso)
                if (attacco.Value.GiocatoriPartecipanti.ContainsKey(username))
                {
                    ...
                    totGuerrieri += truppe.Guerrieri;
                    totLancieri += truppe.Lancieri;
                    totArcieri += truppe.Arcieri;
                    totCatapulte += truppe.Catapulte;
                    ...
                    {
                        attaccoTotG += part.Value.Guerrieri;
                        attaccoTotL += part.Value.Lancieri;
                        attaccoTotA += part.Value.Arcieri;
